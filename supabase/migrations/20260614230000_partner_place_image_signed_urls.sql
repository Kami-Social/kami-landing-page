-- Partner portal: stop emitting broken public place-images URLs; expose storage paths;
-- allow authenticated partner members to read linked venue images from private bucket.
...
create or replace function public.kami_resolve_place_photo_storage(p_place_id uuid)
returns jsonb
language sql
stable
security definer
...
as $$
  select jsonb_build_object(
    'storage_bucket', coalesce(nullif(trim(pi.storage_bucket), ''), 'place-images'),
    'storage_path', pi.storage_path
  )
  from public.place_images pi
  where pi.place_id = p_place_id
...
revoke all on function public.kami_resolve_place_photo_storage(uuid) from public;
grant execute on function public.kami_resolve_place_photo_storage(uuid) to authenticated;

-- Patch venue payload in dashboard RPC (storage paths for client-side signed URLs).
...
-- Partner members may read approved place images for venues linked to their account.
drop policy if exists "Partner members read linked place images" on storage.objects;

create policy "Partner members read linked place images"
  on storage.objects
  for select
  to authenticated
  using (
    ...
    and exists (
      select 1
       ...
       where pi.status = 'approved'
         and pi.storage_path = storage.objects.name
    )
  );

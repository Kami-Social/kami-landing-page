-- When a visit expires with only a brief ping burst, credit the full presence
-- session window (typically 30 min) instead of seconds between first/last ping.
        ...
        round(
          greatest(
            ...
            least(
              extract(epoch from (
                case
                  when not v.is_active_now
                    and v.expires_at <= now()
                    and (v.expires_at - v.first_seen_at) > interval '0 minutes'
                    and (v.last_seen_at - v.first_seen_at)
                      < (v.expires_at - v.first_seen_at) * 0.25
                  then v.expires_at
                  else least(
...
-- Keep admin partner visit history in sync.
create or replace function public.kami_admin_partner_venue_visit_history(
  p_partner_id uuid,
  p_place_id uuid,
  p_search text default null,
  p_visit_filter text default 'all',
...
begin
  ...
  if not exists (
    ...
    raise exception 'venue_not_linked' using errcode = '42501';
  end if;
  ...
  with visits as (
    select
      ...
      bool_or(upp.status = 'active' and upp.expires_at > now()) as is_active_now,
      (array_agg(upp.status order by upp.last_seen_at desc))[1] as last_status,
      (array_agg(upp.source order by upp.last_seen_at desc))[1] as last_source
    from public.user_place_presence upp
    where upp.place_id = p_place_id
      and upp.event_id is null
      and upp.status in ('active', 'demo', 'expired')
  ...
  filtered as (
    ...
    where u.is_removed = false
      and (
        ...
        or lower(coalesce(up.display_name, '')) like '%' || v_search || '%'
        or lower(coalesce(u.ig_handle, '')) like '%' || v_search || '%'
        or lower(coalesce(u.auth_email, '')) like '%' || v_search || '%'
      )
      and (
        v_filter = 'all'
        or (v_filter = 'active_now' and v.is_active_now)
